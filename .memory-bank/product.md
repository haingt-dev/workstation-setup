...
3. [`setup.sh`](../../setup.sh:1) orchestrates:
   ...
   - **Agent Hub** via [`scripts/agent_setup.sh`](../../scripts/agent_setup.sh:1) (clones `~/Projects/agent/` from GitHub)
...
- **Selective skipping**: In default mode, skip specific parts via `--skip-qdrant`, `--skip-godot`, `--skip-apps`, `--skip-easyeffects`, `--skip-packettracer`.
