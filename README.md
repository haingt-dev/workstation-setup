...
- **Own widgets**: `dev.haint.dashboard` on the desktop (clock + date, CPU/RAM/GPU/Disk cards, HCMC weather from Open-Meteo) and `dev.haint.claudequota` in the dock (ring gauge of the worst Claude Code rate-limit window, popup with every window and its reset time — including the per-model weekly limit that nothing else exposes). Because it already polls every minute all day, it is also the only thing that can see a week's true peak, so it keeps a per-day maximum of each window in `~/.local/share/workstation-setup/claude-quota-history.json` — under `share`, not `cache`, so a cache sweep cannot erase it. That file is what makes a quarterly "hold the plan or upgrade" review readable at all: the endpoint only ever reports the window it is in right now, and Claude Code prunes its transcripts after about five weeks. Both read the generated tokens, so they match everything else by construction.
