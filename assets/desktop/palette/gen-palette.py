...
# The starship prompt is written against CATPPUCCIN'S semantic colour names.
# Rather than rewrite the config, we hand it the same names filled with OUR
# colours — so its layout, which is fine, keeps working and only the palette
# changes.
def catppuccin_names(t: dict[str, int], ansi: dict[int, int]) -> dict[str, str]:
    m = {
        ...
        # one pure #000000 on a dark scheme, and a powerline prompt painted in
        # it reads as solid black blocks punched into the terminal. `base` — the
        # prompt's main background — is the same surfaceContainer the dock and
